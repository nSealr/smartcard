...
- Optional PC/SC transport boundary that exchanges short APDUs through
  ...
  rejects malformed reader responses whose data or status bytes are outside the
  APDU byte range. It is tested with fake connections; no real card support is
  claimed yet.
